define(function(require) {
	...
	var app = {
		...
		subscribe: {
			'userdashboard.initModules': 'callLogsInitModuleLayout'
		},

		callLogsInitModuleLayout: function(args) {
			var self = this;
		...
		callLogsRender: function(args) {
			...
			self.callLogsGetData(function() {
				self.callLogsRenderContent(args.container, args.fromDate, args.toDate, args.type, args.callback);
			});
		},
		...
		callLogsGetData: function(globalCallback) {
			...
			monster.parallel({
				...
				vars.devices = _.keyBy(results.devices, 'id');
		...
		callLogsBindEvents: function(params) {
			...
			template.find('.apply-filter').on('click', function(e) {
				...
				self.callLogsRenderContent(container, fromDate, toDate, 'custom');
			});
			...
			template.find('.fixed-ranges button').on('click', function(e) {
				...
				if (type !== 'custom') {
					...
					self.callLogsRenderContent(container, dates.from, dates.to, type);
				} else {
		...
		callLogsFormatCdrs: function(cdrs) {
			var self = this,
				...
				formatCdr = function(cdr) {
					...
					if (cdr.hasOwnProperty('custom_channel_vars') && cdr.custom_channel_vars.hasOwnProperty('authorizing_id') && vars.devices.hasOwnProperty(cdr.custom_channel_vars.authorizing_id)) {
						var device = vars.devices[cdr.custom_channel_vars.authorizing_id];

						call.formatted = call.formatted || {};
						call.formatted.deviceIcon = deviceIcons[device.device_type];
						call.formatted.deviceTooltip = self.i18n.active().userdashboard.submodules.callLogs.devicesTypes[device.device_type];
		...
		callLogsShowDetailsPopup: function(callLogId) {
			...
			self.callApi({
				...
				success: function(data, status) {
					...
					monster.ui.dialog(template, { title: self.i18n.active().userdashboard.submodules.callLogs.detailsPopupTitle });
				},
				error: function(data, status) {
					monster.ui.alert('error', self.i18n.active().userdashboard.submodules.callLogs.alertMessages.getDetailsError);
				}
			});
		},
